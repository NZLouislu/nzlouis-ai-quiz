...
export async function POST(req: Request) {
  try {
    const { mode, question, options, selectedAnswer, messages = [] as ChatMessage[] } = await req.json();
...
`;
    ...
    if (mode === "initial") {
      ...
      const history = (messages as ChatMessage[]).map((m) => `${m.role.toUpperCase()}: ${m.content}`).join("\n");
    ...
    if (!apiKey || !apiUrl) return NextResponse.json({ error: "API key or URL not set" }, { status: 500 });

    const payload = { contents: [{ parts: [{ text: base + "\n" + userText }] }] };
    ...
    return NextResponse.json({ error: "AI Assistant failed", details: err instanceof Error ? err.message : String(err) }, { status: 500 });
  }
}